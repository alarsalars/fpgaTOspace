...
// WE test the median filter with the tx&rx IP
#include <iostream>
#include <fstream>
#include <unistd.h>
#include <stdlib.h>
...
int main (){
    ...
    stream_data rx_st,median, tx_st;
    uint16_t length = 4 ;
    uint16_t width = 64;
    ap_uint<256> combined_value;
    // We load the AXI burst array with a random array throughout the whole array 
    for (_loop i = 0; i < length; i++) {
        for (_loop j = 0; j < width; j++) {
            ap_uint<64> part1 = distr(gen);
            ap_uint<64> part2 = distr(gen);
            ap_uint<64> part3 = distr(gen);
            ap_uint<64> part4 = distr(gen);
            combined_value(255, 192) = part1;
            combined_value(191, 128) = part2;
            combined_value(127, 64) = part3;
            combined_value(63, 0) = part4;
            array_TX[i * width + j] = combined_value;
...
// for AXI DMA Median
    ...
    cout << " stream IN OUT MEDIAN"<<endl;
    median_filter(tx_st, length, width,median );
    cout << " Stream to AXI with RX IP "<<endl;
    AXIDMA_RX(median, length, width, array_RX);   





// this for TX and RX function combined
