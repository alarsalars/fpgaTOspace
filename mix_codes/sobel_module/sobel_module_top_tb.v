...
sobel_module_TOP sobel_module_top(
        .CLK(clk),                  
        .RST(rst),            
        // INPUT                
        .R_cam_i(red_i),              
...
// -------------------------------THE Gray results array ----------------------------------
    integer i=0;
    integer j=0;
    localparam results_array_len =  1000*1024; 
    ...
    always @(posedge clk) begin
        if (rst) begin 
            ...
            if(done_o)begin // load the new data to the new array
                results_array[j]   <= red_o;
                results_array[j+1] <= green_o;
                results_array[j+2] <= blue_o;
                j=j+3;
            end
        end
...
//---------------------------------------------------------------
    `define write_filename "C:\\Users\\TAlars\\Downloads\\results22.bmp"
