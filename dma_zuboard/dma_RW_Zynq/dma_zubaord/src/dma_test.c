...
int main(){
	...
	Xil_DCacheFlushRange((u32)a,50*sizeof(u32));
	Xil_DCacheFlushRange((u32)b,50*sizeof(u32));
	status = XAxiDma_SimpleTransfer(&myDma, (u32)b, 50*sizeof(u32),XAXIDMA_DEVICE_TO_DMA);
	status = XAxiDma_SimpleTransfer(&myDma, (u32)a, 50*sizeof(u32),XAXIDMA_DMA_TO_DEVICE);
	if(status != XST_SUCCESS){
		print("DMA initialization failed\n");
		return -1;
	}
